...
// ==========================================================================
// FUNCIONALIDAD DE EXPANSIÓN / CONTRACCIÓN DE TENDENCIA DIARIA
// ==========================================================================
document.getElementById('btn-expand-tendencia').addEventListener('click', function () {
    const chartsRow = document.getElementById('chartsRow');
    const btnIcon = this.querySelector('i');

    // Alternar la clase expansora en el grid
    chartsRow.classList.toggle('tendencia-expanded');

    const isExpanded = chartsRow.classList.contains('tendencia-expanded');

    // Cambiar dinámicamente el icono del botón
    if (isExpanded) {
        btnIcon.className = "fa-solid fa-compress";
        this.setAttribute('title', 'Contraer');
    } else {
    ...
    const intervalResize = setInterval(() => {
        ...
        pasadas++;
        // Sigue redibujando durante los 400ms que dura la transición CSS para un efecto ultra fluido
        if (pasadas > 20) clearInterval(intervalResize);
    }, 20);
});
